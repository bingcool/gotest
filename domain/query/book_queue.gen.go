...
func newBookQueue(db *gorm.DB, opts ...gen.DOOption) bookQueue {
	...
	_bookQueue.bookQueueDo.UseModel(&model.BookQueue{})
...
func (b bookQueueDo) Create(values ...*model.BookQueue) error {
	if len(values) == 0 {
		return nil
	}
...
func (b bookQueueDo) CreateInBatches(values []*model.BookQueue, batchSize int) error {
	return b.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (b bookQueueDo) Save(values ...*model.BookQueue) error {
	if len(values) == 0 {
		return nil
	}
...
func (b bookQueueDo) First() (*model.BookQueue, error) {
	if result, err := b.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*model.BookQueue), nil
	}
}

func (b bookQueueDo) Take() (*model.BookQueue, error) {
...
func (b bookQueueDo) Last() (*model.BookQueue, error) {
...
func (b bookQueueDo) Find() ([]*model.BookQueue, error) {
	result, err := b.DO.Find()
	return result.([]*model.BookQueue), err
}

func (b bookQueueDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*model.BookQueue, err error) {
	buf := make([]*model.BookQueue, 0, batchSize)
...
func (b bookQueueDo) FindInBatches(result *[]*model.BookQueue, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (b bookQueueDo) FirstOrInit() (*model.BookQueue, error) {
...
func (b bookQueueDo) FirstOrCreate() (*model.BookQueue, error) {
...
func (b bookQueueDo) FindByPage(offset int, limit int) (result []*model.BookQueue, count int64, err error) {
...
func (b bookQueueDo) Delete(models ...*model.BookQueue) (result gen.ResultInfo, err error) {
